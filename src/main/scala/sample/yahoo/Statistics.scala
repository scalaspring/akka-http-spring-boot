...
trait Statistics[T] {
  ...
  def update(value: T): Statistics[T]
}

object Statistics {

  def apply[T](population: Boolean = false)(implicit num: Numeric[T]): Statistics[T] = new StatisticsImpl[T](population)(num)
  ...
  private case class StatisticsImpl[T](population: Boolean = false, M: Double = 0, S: Double = 0, k: Int = 1)(implicit num: Numeric[T]) extends Statistics[T] {
    ...
    override def update(value: T): Statistics[T] = {
